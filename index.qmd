...
::: {.fragment .fade-in-then-semi-out}
-   Can sometimes better encode information [@barfield1989].
:::
...
stimuli %>% 
  ...
  geom_text(aes(label = round(values, 1), y = 5), size = 5) + 
  theme_minimal() + 
  annotate('text', x = 5, y = 25, label = '*', size = 8) +
...
stimuli %>% 
  ...
  geom_bar(stat = 'identity', width = 1, color = 'black', fill = '#B50000') + 
  geom_text(aes(label = round(ratio, 2), y = 0.125/2), size = 5) + 
  theme_minimal() + 
